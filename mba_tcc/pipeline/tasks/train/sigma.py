...
from mba_tcc.utils.config import DEFAULT_VAL_COLUMN, DEFAULT_PREDICTED_VALUE
...
def calculate_zscores(train_file: pd.DataFrame, step: int, condition: Callable) -> Tuple[pd.DataFrame, int]:
    ...
    result = result.rename(columns={zscores_column: DEFAULT_PREDICTED_VALUE})
...
def sigma_method(train_file: pd.DataFrame, output_path: Path, params: dict, condition: Callable, step: int = 3) -> bool:
